...
```
物料名称 | CAS号 | 化学式 | 分子量 | 物料备注 | 价格 | 价格单位 | 数量级 | 供应商 | 日期 | 规格 | 含量 | 价格备注
```

- 表头按别名容错匹配；**CAS 号必填**。
- 按 CAS 号导入物料：已存在的 CAS 会更新其「物料名称 / CAS号 / 化学式 / 分子量 / 物料备注」。
- 其余列写入价格记录：价格、价格单位、数量级、供应商、日期、规格、含量、价格备注。
- 「数量级」取值范围为 千克 / 十千克 / 百千克 / 吨（可留空）。模板里这一列带下拉；
  导入时也接受「KG」「1吨」这类等价写法，认不出的写法会报错并跳过该条价格。
- 日期同时支持字符串（`2026-08-25`）与 Excel 序列号。
- 备注分两列各自记录：`物料备注` 写物料，`价格备注` 写这条价格记录。
- 拆分前的模板只有一个「备注」列，导入时**按价格备注处理**，老文件仍可直接导入。
...
本项目自有代码以 **[PolyForm Noncommercial License 1.0.0](LICENSE)** 授权。

简单说：**仅限非商业用途**。个人学习、研究、试验、业余项目，以及慈善组织、教育机构、
公共研究机构、公共安全与卫生机构、环保组织、政府机构的使用，都在许可范围内；
**任何商业用途都需要另行取得授权**，请联系版权人。

分发时必须把许可条款（或指向它的 URL）与本项目的 `Required Notice` 行一并转交。
软件按「原样」提供，不附带任何担保。

这不是一个 OSI 认可的开源许可证——它是「源码可见」，不是「开源」。

第三方依赖与打包资源的许可（与上述条款互不影响）见 [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)。

> 各版本安装包均**未做代码签名**，与许可无关，详见 [下载安装](#下载安装)。
