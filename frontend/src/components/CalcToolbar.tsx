...
/** 计算工具栏：操作按钮 + 汇总统计 */
function CalcToolbar({ result, calculating, onAddStep, onSave, onLoad, onRecalculate, onClear }: Props) {
  const totalCost = result?.totalCost || 0
  const totalUnitCost = result?.totalUnitCost || 0
  const totalYield = result?.totalYieldKg || 0
